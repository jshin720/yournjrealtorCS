...
namespace plainview\sdk_eightb_home_value\wordpress\form2;
...
**/
class form
	extends \plainview\sdk_eightb_home_value\form2\form
{
	public $base;
	...
	public function __construct( $base )
	{
		...
		) as $input )
		{
			$o = new \stdClass();
			$o->name = $input;
			$o->class = sprintf( '\\plainview\\sdk_eightb_home_value\\wordpress\form2\\inputs\\%s', $input );
	...
	**/
	...
	{
		$o = \plainview\sdk_eightb_home_value\base::merge_objects( array(
			'base' => $this->base,
			'header' => '',
			'header_level' => 'h3',
			'r' => '',					// Return value.
	...
	public function display_form_table_inputs( $o )
	{
		...
		foreach( $o->inputs as $input )
		{
			...
			$container = false;
			$container |= is_subclass_of( $input, 'plainview\\sdk_eightb_home_value\\form2\\inputs\\inputfieldset' );
			$uses = class_uses( $input );
			$container |= isset( $uses[ 'plainview\\sdk_eightb_home_value\\form2\\inputs\\traits\\container' ] );
			...
			if ( is_a( $input, 'plainview\\sdk_eightb_home_value\\form2\\inputs\\markup' ) )
			{
	...
	/**
		...
		@see		\\plainview\\sdk_eightb_home_value\\form2\\form::_
